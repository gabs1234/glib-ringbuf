...
const gsize bit_depth = sizeof (guint8);
...
const guint ring_buf_length = nb_images * nb_pixels;
...
gpointer writer_thread (gpointer data) {
    guint8 *buf = g_malloc (image_size);
    guint8 i = 0;
...
int main (int argc, char **argv) {
    ...
    g_print ("Creating\n");
    rb = ringbuf_new (bit_depth, ring_buf_length);
